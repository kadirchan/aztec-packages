...
export class Sequencer {
  ...
  protected async work() {
    try {
      ...
      await this.buildNextBlock(newGlobalVariables, historicalHeader);
    } catch (err) {
  ...
  @trackSpan('buildNextBlock', (globalVariables, header) => ({
    [Attributes.BLOCK_PARENT]: header?.globalVariables.blockNumber.toNumber(),
    [Attributes.BLOCK_NUMBER]: globalVariables.blockNumber.toNumber(),
  }))
  private async buildNextBlock(globalVariables: GlobalVariables, historicalHeader: Header | undefined): Promise<void> {
    const txs = await this.getTxsFromP2P(globalVariables);
    if (txs.length < this.minTxsPerBLock) {
      return;
    }

    const { block, aggregationObject, proof } = await this.assembleBlock(txs, globalVariables, historicalHeader);
    await this.publishL2Block(block, aggregationObject, proof);
  }

  @trackSpan('getTxsFromP2P')
  private async getTxsFromP2P(globalVariables: GlobalVariables): Promise<Tx[]> {
    ...
    const allValidTxs = await this.takeValidTxs(
      pendingTxs,
      this.txValidatorFactory.validatorForNewTxs(globalVariables, this.allowedInSetup),
    );
  ...
  @trackSpan(
    'assembleBlock',
    (txs, globalVariables, historicalHeader) => ({
      [Attributes.BLOCK_PARENT]: historicalHeader?.globalVariables.blockNumber.toNumber(),
      [Attributes.BLOCK_NUMBER]: globalVariables.blockNumber.toNumber(),
      [Attributes.BLOCK_CANDIDATE_TXS_COUNT]: txs.length,
    }),
    blockResult => ({
      [Attributes.BLOCK_TXS_COUNT]: blockResult.block.body.txEffects.length,
    }),
  )
  private async assembleBlock(
    ...
    const newBlockNumber = newGlobalVariables.blockNumber.toNumber();
    ...
    if (processedTxs.length === 0) {
      ...
      this.prover.cancelBlock();
      throw new Error('No txs processed correctly to build block');
    }
    ...
    this.log.verbose(`Assembled block ${blockResult.block.number}`, {
      eventName: 'l2-block-built',
      duration: workTimer.ms(),
      publicProcessDuration: publicProcessorDuration,
      rollupCircuitsDuration: blockBuildingTimer.ms(),
      ...blockResult.block.getStats(),
   ...
   */
  @trackSpan('publishL2Block', block => ({
